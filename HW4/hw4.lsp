...
; takes an addition start parameter to implement recursion
;
(defun at-most-one-color-helper (n start c k)
  (cond ((= c k) nil)
	(t (cons (append (list (- (node2var n start k))) (list (- (node2var n (+ c 1) k)))) (at-most-one-color-helper n start (+ c 1) k)))
  )
)
...
; takes an additional start parameter to implement recursion
;
(defun generate-edge-clauses-helper (e start k)
  (cond ((> start k) nil)
	(t (cons (append (list (- (node2var (first e) start k))) (list (- (node2var (second e) start k)))) (generate-edge-clauses-helper e (+ start 1) k)))
  )
)
